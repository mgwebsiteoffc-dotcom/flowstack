...
<div class="max-w-3xl mx-auto">
    @if ($canManage)
        <div class="bg-white rounded-xl border border-gray-100 p-5 mb-6">
            <h3 class="font-semibold text-gray-900 mb-3">📣 New announcement</h3>
            <form method="POST" action="{{ route('announcements.store') }}" class="space-y-3">
                ...
                <div class="flex items-center gap-4 text-sm text-gray-600">
                    <label class="flex items-center gap-2"><input type="checkbox" name="is_pinned" value="1" class="rounded"> 📌 Pin</label>
                ...
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <h4 class="font-semibold text-gray-900 flex items-center gap-2">
                            @if ($announcement->is_pinned)<span class="text-xs bg-amber-100 text-amber-700 rounded-full px-2 py-0.5">📌 Pinned</span>@endif
                            {{ $announcement->title }}
                        </h4>
