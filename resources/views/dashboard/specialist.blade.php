...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    <x-stat-card title="My Open Tasks" value="{{ $stats['my_tasks'] }}" icon="✅" color="indigo" />
    <x-stat-card title="Overdue" value="{{ $stats['overdue'] }}" icon="⏰" color="{{ $stats['overdue'] > 0 ? 'red' : 'green' }}" />
    <x-stat-card title="Due Today" value="{{ $stats['due_today'] }}" icon="📅" color="blue" />
    <x-stat-card title="Hours This Week" value="{{ $stats['hours_this_week'] }}" icon="⏱️" color="purple" />
</div>

<div class="mt-6">
    <x-card title="My Tasks" icon="✅">
        ...
        @empty
            <x-empty-state icon="✅" title="No tasks assigned" message="Tasks assigned to you will show up here." :action="route('tasks.index')" actionLabel="Browse tasks" />
        @endforelse
    </x-card>
</div>
@endsection
