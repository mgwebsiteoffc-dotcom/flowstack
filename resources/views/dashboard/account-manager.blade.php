...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    <x-stat-card title="My Clients" value="{{ $stats['clients'] }}" icon="🤝" color="indigo" />
    <x-stat-card title="Open Tasks" value="{{ $stats['open_tasks'] }}" icon="✅" color="blue" />
    <x-stat-card title="Pending Approvals" value="{{ $stats['pending_approvals'] }}" icon="⏳" color="amber" />
    <x-stat-card title="Reports in Draft" value="{{ $stats['reports_due'] }}" icon="📈" color="purple" />
</div>

<div class="grid lg:grid-cols-2 gap-6 mt-6">
    <x-card title="My Clients" icon="🤝">
        ...
        @empty
            <x-empty-state icon="🤝" title="No clients assigned" message="Clients assigned to you will appear here." />
        @endforelse
    </x-card>
    ...
    <x-card title="Tasks Due Today" icon="📅">
        ...
        @empty
            <p class="text-sm text-gray-400 py-3 text-center">No tasks due today 🎉</p>
        @endforelse
    </x-card>
</div>
@endsection
