...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
    <x-stat-card title="Monthly Revenue" :value="auth()->user()->canViewFinancials() ? '₹'.number_format($stats['monthly_revenue']) : '••••'" icon="banknotes" color="green" />
    ...
    <x-stat-card title="Pipeline Value" :value="auth()->user()->canViewFinancials() ? '₹'.number_format($stats['pipeline_value']) : '••••'" icon="target" color="purple" />
</div>
        ...
        <x-card title="Client Health" icon="pulse">
            @forelse ($clients as $client)
                <a href="{{ route('clients.show', $client) }}" class="flex items-center gap-3 py-2.5 border-b border-gray-50 last:border-0 hover:bg-gray-50 rounded-lg px-2">
                    ...
                    <span class="text-xs font-medium"><x-financial>₹{{ number_format($client->monthly_retainer ?? 0) }}</x-financial></span>
                </a>
            @empty
